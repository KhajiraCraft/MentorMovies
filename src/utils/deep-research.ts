...
export function getSystemPrompt() {
  const now = new Date().toISOString();
  return `You are a movie recommendation researcher. Today is ${now}. Your job is to find movies recommended or liked by influential people.

Follow these instructions:
- Assume the influencer mentioned by the user is correct.
- Do deep and broad web research to find mentions of movies the influencer has recommended, mentioned, or liked.
- You may use interviews, tweets, articles, or any verified online content.
- Output should only include confirmed movies.
- Be accurate, thorough, and avoid speculation unless explicitly stated.
- The final output will be in JSON, listing movie titles and relevant context (e.g., quote, source).
- Do not write a report or explanations.
- Do not ask follow-up questions.`;
}

export function getOutputGuidelinesPrompt() {
  return `<OutputGuidelines>
Only return a list of movies in JSON format. Do not use markdown formatting, headings, or explanatory text. Avoid quotes, math, or any styling. Output should be clean and valid JSON only.
</OutputGuidelines>`;
}
      ...
      z
        .object({
          ...
          researchGoal: z
            .string()
            .describe(
              "State that the goal is to find movies recommended, praised, or liked by the influencer. Mention that we are looking for reliable sources like tweets, interviews, or articles that include movie mentions. JSON reserved words should be escaped."
            ),
        })
        .required({ query: true, researchGoal: true })
    )
...
export function generateSerpQueriesPrompt(query: string) {
  ...
  return [
`The user wants to know what movies have been recommended, liked, or mentioned by the following person:\n<query>${query}</query>`,
    `Generate unique and diverse Google search queries that will help find credible mentions of movies by this person. Examples include quotes, interviews, social media posts, or curated lists.`,
    `You MUST respond in \`JSON\` matching this \`JSON schema\`:\n\`\`\`json\n${outputSchema}\n\`\`\``,
    `Expected output:\n\`\`\`json\n[{query: "elon musk favorite movies", researchGoal: "This is a sample query."}]\n\`\`\``,
  ].join("\n\n");
}

export function processResultPrompt(query: string, researchGoal: string) {
  return [
    `Use the following query to gather evidence of movie recommendations by the person:\n<query>${query}</query>`,
    `Focus on identifying exact movie titles the person has praised or recommended. Use reliable sources like interviews, tweets, blog posts, or articles.`,
    `Output a JSON array of objects. Each object should include the movie title (use the official title if possible), release year, and context. Only include movies that were directly or strongly associated with the influencer.`,
    `Example format:\n\`\`\`json\n[\n  {\n    "title": "The Matrix",\n    "year": 1999,\n    "context": "Elon Musk said on Twitter: 'The Matrix is a documentary.'" \n  }\n]\n\`\`\``,
  ].join("\n\n");
}

// 6. Process search results to extract movie names
export function processSearchResultPrompt(query: string, researchGoal: string) {
  return [
    `Use the following query to gather evidence of movie recommendations by the person:\n<query>${query}</query>`,
    `Focus on identifying exact movie titles the person has praised or recommended. Use reliable sources like interviews, tweets, blog posts, or articles.`,
    `Output a JSON array of objects. Each object should include the movie title (use the official title if possible) and the source. Only include movies that were directly or strongly associated with the influencer.`,
    `Example format:\n\`\`\`json\n[\n  {\n    "title": "The Matrix",\n "date": "1999",\n    "context": "Elon Musk said on Twitter: 'The Matrix is a documentary.'" \n  }\n]\n\`\`\``,
  ].join("\n\n");
}
...
export function reviewSerpQueriesPrompt(
  ...
  const learningsString = learnings
    ...
    .join("\n");
    return [
      `User is researching the following person:\n<query>${query}</query>`,
      `Here are the current findings:\n<learnings>\n${learningsString}\n</learnings>`,
      `User's suggestion:\n<suggestion>\n${suggestion}\n</suggestion>`,
      `If necessary, provide more search queries to continue finding movie recommendations.`,
      `Respond in \`JSON\` matching this \`JSON schema\`:\n\`\`\`json\n${outputSchema}\n\`\`\``,
    ].join("\n\n");
}

// 8. Final report is no longer required
export function writeFinalReportPrompt(query: string, learnings: string[]) {
  const learningsString = learnings
    .map((learning) => `<learning>\n${learning}\n</learning>`)
    .join("\n");
  return [
    `You do NOT need to write a final report.`,
    `Instead, summarize the results into a JSON array of recommended movies with brief context. Include the movie title, release year, and context.`,
    `User's query:\n<query>${query}</query>`,
    `Here are the findings:\n<learnings>\n${learningsString}\n</learnings>`,
    `Respond in the following format:\n\`\`\`json\n[\n  { "title": "Interstellar", "year": 2014, "context": "Elon Musk recommended it in an interview." }\n]\n\`\`\``,
  ].join("\n\n");
}
