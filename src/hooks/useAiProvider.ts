...
//           baseURL: completePath(GEMINI_BASE_URL, "/v1beta"),
//           apiKey: "AIzaSyBUERTG04ewg5ehf1pHHeAuvB-eY37R__E",
//         });
...
function useModelProvider() {
  ...
  function getModel() {
    ...
    switch (provider) {
      ...
      case "openai":
        ...
        return { thinkingModel: openAIThinkingModel, networkingModel: openAINetworkingModel };
      case "anthropic":
        const { anthropicThinkingModel, anthropicNetworkingModel } = useSettingStore.getState();
        return { thinkingModel: anthropicThinkingModel, networkingModel: anthropicNetworkingModel };
      case "deepseek":
        const { deepseekThinkingModel, deepseekNetworkingModel } = useSettingStore.getState();
        return { thinkingModel: deepseekThinkingModel, networkingModel: deepseekNetworkingModel };
      case "xai":
        const { xAIThinkingModel, xAINetworkingModel } = useSettingStore.getState();
        return { thinkingModel: xAIThinkingModel, networkingModel: xAINetworkingModel };
      case "openrouter":
        const { openRouterThinkingModel, openRouterNetworkingModel } = useSettingStore.getState();
        return { thinkingModel: openRouterThinkingModel, networkingModel: openRouterNetworkingModel };
      case "openaicompatible":
        const { openAICompatibleThinkingModel, openAICompatibleNetworkingModel } = useSettingStore.getState();
        return { thinkingModel: openAICompatibleThinkingModel, networkingModel: openAICompatibleNetworkingModel };
      case "ollama":
        const { ollamaThinkingModel, ollamaNetworkingModel } = useSettingStore.getState();
        return { thinkingModel: ollamaThinkingModel, networkingModel: ollamaNetworkingModel };
      default:
        throw new Error("Unsupported Provider: " + provider);
    }
  }
  ...
  function hasApiKey(): boolean {
    // Always return true since all requests go through the backend now
    return true;
  }
