...
function FinalReport() {
  ...
  //   try {
  //     accurateTimerStart();  // Start timer
  //     setIsWriting(true);    // Show loading UI
  ...
  //     // Generate Final Report and update Zustand store
  //     await writeFinalReport();
  
  //     // Fetch latest data from Zustand
  //     const { finalReport, sources, tasks } = useTaskStore.getState();
  
  //     // Extract Movies from tasks
  //     const movies = tasks.map((task) => ({
  //       title: task.query,       // Task query = Movie Title
  //       context: task.learning,  // Task learning = Context or Source Info
  //     }));
  ...
  //     if (!response.ok) {
  //       console.error("❌ Failed to save final report to API:", await response.json());
  //     } else {
  //       console.log("✅ Final report successfully sent to API.");
  //     }
  
  //   } catch (error) {
  //     console.error("❌ Error during report generation or API call:", error);
  //   } finally {
  //     setIsWriting(false);  // Reset loading state
  //     accurateTimerStop();  // Stop timer
  //   }
  // }
  ...
  async function handleSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      try {
        // Attempt to extract movie JSON from finalReport
        const start = finalReport.indexOf("[");
        const end = finalReport.lastIndexOf("]") + 1;
  
        if (start !== -1 && end !== -1 && start < end) {
          const jsonString = finalReport.slice(start, end);
          movies = JSON.parse(jsonString);
        } else {
          console.warn("⚠️ No JSON array found in finalReport.");
        }
      } catch (err) {
        console.error("❌ Failed to parse movies from finalReport:", err);
      }
      ...
      const response = await fetch("/api/movies", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ finalReport, movies }),
      });
